...
module lever_arm(anchor=CENTER, spin=0, orient=UP) {
    arm_hole_displacement = [-17.5, 0, 0];
        ...
        difference() {
            ...
            cyl(d=joint_outer_diam, l=overall_thickness) {
                ...
                tube(id=linkage_arm_hole_diam, od=linkage_arm_joint_diam, l=linkage_arm_thickness)
                    ...
                    position(RIGHT)
                    right((joint_outer_diam + 1 - linkage_arm_joint_diam) / 2)
                    cube([joint_outer_diam + 1 - linkage_arm_joint_diam, linkage_arm_width, linkage_arm_thickness], anchor=RIGHT) {
                        // main body
                        position(RIGHT+BACK)
                        cube([9, 9, overall_thickness], anchor=LEFT+BACK) {
                            // lever arm
                            position(LEFT+FRONT)
                            zrot(-25)
